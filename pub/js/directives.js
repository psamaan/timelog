...
timelogDirectives.directive('alert', function($interval, $timeout, $rootScope){
    return {
        restrict: 'E',
        template: '<div class="alert animate-if text-center col-sm-4" ng-class="alertMessage[0].class" style="position:fixed;" ng-if="alertMessage[0].text">' + '{{alertMessage[0].text}}' + '</div>',
        link: function (scope) {
